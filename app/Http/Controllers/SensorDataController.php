...
class SensorDataController extends Controller
{
    ...
    public function inserirSensorData(Request $request)
    {
        ...
        if (!$request->user() || !$request->user()->tokenCan('escotilha:send-data')) {
            return ApiResponse::unauthorized();
        }

        try {
            $escotilha = Escotilha::where('serial_number', $request->serial_number)->firstOrFail();
    ...
    {
        if (!auth()->user() || !auth()->user()->tokenCan('admin,enterprise')) {
            return ApiResponse::unauthorized();
        }
    ...
    public function deletarSensorData($id)
    {
        ...
        try {
            $registro = SensorData::findOrFail($id);
            $registro->delete();
